...
`fg` 只接受严格的 `#RGB` 或 `#RRGGBB`，`bold` 和 `dim` 接受布尔值。省略字段会保留上下文样式；显式 `false` 会移除对应修饰。样式只作用于当前出现位置。为 `git_status` 设置 `fg` 后，未提交更改数量会使用该颜色，而不是默认的蜜桃色。token 样式不会改变分隔符或行背景。
