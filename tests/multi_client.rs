...
fn non_foreground_client_render_preserves_agent_panel_scroll() {
    ...
    // Expected labels track our local sidebar defaults (35/65 split, row_gap 1), so they
    // differ from upstream's; recompute them from a failing frame if those defaults change.
    ...
    let (reached_bottom, setup_frames) = wait_for_frame_matching_with_snapshots(
        ...
        |frame| agent_panel_starts_with(frame, "agent-12"),
    )
    ...
    let (started_at_tall_limit, initial_frames) =
        wait_for_frame_matching_with_snapshots(&mut probe, Duration::from_secs(3), |frame| {
            agent_panel_starts_with(frame, "agent-04")
        })
    ...
    let (scrolled, probe_frames) =
        wait_for_frame_matching_with_snapshots(&mut probe, Duration::from_secs(3), |frame| {
            agent_panel_starts_with(frame, "agent-05")
        })
