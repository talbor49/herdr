...
/// Per-entry clickable card geometry for the agent panel, computed with the
/// same variable-height + gap layout the renderer uses so the two never drift.
/// Each card rect spans the entry's rendered rows, making the whole entry
/// clickable.
pub(crate) fn compute_agent_panel_card_areas(
    ...
    // Clamp a stale scroll offset (e.g. after a dynamic agent-view filter shrinks
    // the list) so the visible window still lands on real entries.
    ...
    for (index, entry) in entries.iter().enumerate().skip(scroll) {
        ...
        cards.push(crate::app::state::AgentPanelCardArea {
            ws_idx: entry.ws_idx,
...
fn agent_panel_visible_count_from(app: &AppState, area: Rect, scroll: usize) -> usize {
    ...
    // Entries are variable height (configurable token rows differ per agent),
    // so count how many fit using their actual heights.
...
"##,
    ...
    fn default_space_workspace_style_tracks_active_state() {
        ...
        // Workspace names are tinted per checkout, so the fg is the checkout's
        // stable accent color; active state is tracked via bold + background.
        let accent_one =
            workspace_accent_color(&app.workspaces[0].accent_color_seed(), &app.palette);
        let accent_two =
            workspace_accent_color(&app.workspaces[1].accent_color_seed(), &app.palette);

        let active = buffer[(find_symbol_x(buffer, first_row, 25, "o"), first_row)].style();
        assert_eq!(active.fg, Some(accent_one));
        ...
        assert_eq!(inactive.fg, Some(accent_two));
        assert!(!inactive
            .add_modifier
    ...
    fn navigate_selection_keeps_its_existing_background_beside_active_workspace() {
        ...
        // Tall enough for two workspace cards under the default row gap.
...
"##,
    ...
    fn agent_panel_entries_carry_agent_title() {
        ...
        let entries = agent_panel_entries(&app);
        assert_eq!(entries[0].pane_label.as_deref(), Some("Refactor auth flow"));
    }
